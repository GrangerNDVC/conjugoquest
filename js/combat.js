...
function afficherInvitationCombat(inv, invId) {
    let overlay = document.getElementById('duel-invitation-overlay');
    if (!overlay) {
        overlay = document.createElement('div');
        overlay.id = 'duel-invitation-overlay';
        overlay.style.cssText = 'position:fixed;top:20px;right:20px;width:300px;background:rgba(0,0,0,0.95);border:3px solid #fbbf24;border-radius:15px;padding:20px;z-index:9999;font-family:Cinzel,serif;color:white;';
        document.body.appendChild(overlay);
    }
    overlay.innerHTML = `
        <div style="color:#fbbf24;font-weight:bold;margin-bottom:8px;">⚔️ INVITATION DUEL</div>
        <div style="margin-bottom:4px;"><strong>${inv.inviter}</strong> te défie !</div>
        <div style="color:#94a3b8;font-size:0.85em;margin-bottom:12px;">Niveau ${inv.level} · ${inv.health} vies · ${inv.timeLimit}s</div>
        <div style="display:flex;gap:8px;">
            <button onclick="accepterInvitationCombat('${inv.roomCode}','${invId}')" style="flex:2;padding:8px;background:#4CAF50;border:none;border-radius:6px;color:white;cursor:pointer;font-weight:bold;">⚔️ Accepter</button>
            <button onclick="refuserInvitationCombat('${invId}')" style="flex:1;padding:8px;background:#f44336;border:none;border-radius:6px;color:white;cursor:pointer;">✕</button>
        </div>`;
    overlay.style.display = 'block';
    setTimeout(() => { if (overlay.style.display !== 'none') refuserInvitationCombat(invId); }, 30000);
}
