...
window.addEventListener('beforeunload', () => {
    const userId = SaveSystem.getCurrentUserId();
    if (userId && window.FirebaseDB) {
        // sendBeacon pour garantir l'envoi même en cours de fermeture
        const url = `https://firestore.googleapis.com/v1/projects/classe-de-francais-b3c41/databases/(default)/documents/joueurs/${userId}`;
        navigator.sendBeacon && navigator.sendBeacon('/beacon', JSON.stringify({userId}));
        // Fallback synchrone
        try {
